...
class TestMemberInsertion:
    ...
    def test_insert_member_basic(self, tmp_path):
        ...
        try:
            ...
            # Verify insertion
            cursor.execute(
                "SELECT email, name, status FROM members WHERE id = ?", ("test_id_123",)
            )
    ...
    def test_insert_member_idempotency(self, tmp_path):
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*), name, email_count FROM members WHERE id = ?",
                ("test_id_123",),
            )
            result = cursor.fetchone()
...
class TestAPIFetching:
    ...
    def test_fetch_single_page(self, mock_db, mock_request):
        ...
        assert count == 2, "Should save 2 members from single page"
        assert mock_request.call_count == 1, "Should make one API call"
    ...
    def test_fetch_multiple_pages(self, mock_db, mock_request):
        ...
        assert count == 2, "Should save 2 members from all pages"
        assert mock_request.call_count == 2, "Should make two API calls for pagination"

    @patch("members_to_sqlite.make_ghost_request")
    @patch("members_to_sqlite.get_db_connection")
    def test_fetch_incremental_with_since(self, mock_db, mock_request):
        ...
        count = members_to_sqlite.fetch_and_save_members(since=since_time)

        # Verify the filter was included in the endpoint
        call_args = mock_request.call_args[0][0]
        assert f"filter=updated_at:>'{since_time}'" in call_args, (
            "Should include since filter in API call"
        )
        assert count == 1, "Should save 1 updated member"
    ...
    def test_get_last_sync_time_with_history(self, tmp_path):
        ...
        try:
            ...
            assert result == test_time, (
                f"Should return the last completed sync time, got {result}"
            )
